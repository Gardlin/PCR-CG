    ...
    def forward(self, batch, backbone2d=None):
        ...
        if self.image_feature:
            if self.img_num == 3:
                ...
                src2_point_with_2dfeature = torch.cat(
                    (src2_feats.transpose(1, 0), torch.ones(src2_feats.shape[1], 1).cuda()), dim=-1).detach()
                src3_point_with_2dfeature = torch.cat(
                    (src3_feats.transpose(1, 0), torch.ones(src3_feats.shape[1], 1).cuda()), dim=-1).detach()
                tgt1_point_with_2dfeature = torch.cat(
                    (tgt1_feats.transpose(1, 0), torch.ones(tgt1_feats.shape[1], 1).cuda()), dim=-1).detach()
                tgt2_point_with_2dfeature = torch.cat(
                    (tgt2_feats.transpose(1, 0), torch.ones(tgt2_feats.shape[1], 1).cuda()), dim=-1).detach()
                tgt3_point_with_2dfeature = torch.cat(
                    (tgt3_feats.transpose(1, 0), torch.ones(tgt3_feats.shape[1], 1).cuda()), dim=-1).detach()
                x = x.repeat(1, 129)
                "=================================================================="
                ...
                tgt1_inds3d = batch['tgt1_inds3d']
                tgt2_inds3d = batch['tgt2_inds3d']
                src_valid_map1 = batch['src_valid_map1']
                src_valid_map2 = batch['src_valid_map2']
                tgt_valid_map1 = batch['tgt_valid_map1']
                tgt_valid_map2 = batch['tgt_valid_map2']
                # concat 2d feature

                src_valid_map1 = src_valid_map1.transpose(0, 1).unsqueeze(0)
                src_valid_map2 = src_valid_map2.transpose(0, 1).unsqueeze(0)
                tgt_valid_map1 = tgt_valid_map1.transpose(0, 1).unsqueeze(0)
                tgt_valid_map2 = tgt_valid_map2.transpose(0, 1).unsqueeze(0)

                src1_feature2d = backbone2d(src_color1.unsqueeze(0).cuda()).squeeze(0)  # 1 128 120 160 3d->2d
                src2_feature2d = backbone2d(src_color2.unsqueeze(0).cuda()).squeeze(0)  # 1 128 120 160 3d->2d
                tgt1_feature2d = backbone2d(tgt_color1.unsqueeze(0).cuda()).squeeze(0)  # 1 128 120 160 3d->2d
                tgt2_feature2d = backbone2d(tgt_color2.unsqueeze(0).cuda()).squeeze(0)  # 1 128 120 160 3d->2d
                src1_feature2d = src1_feature2d * src_valid_map1  # 1 128 120 160 3d->2d
                src2_feature2d = src2_feature2d * src_valid_map2  # 1 128 120 160 3d->2d
                tgt1_feature2d = tgt1_feature2d * tgt_valid_map1  # 1 128 120 160 3d->2d
                tgt2_feature2d = tgt2_feature2d * tgt_valid_map2
                # print("src_feature2d=src_feature2d * src_valid_map")
                src1_feats = src1_feature2d[:, src1_inds2d[:, 1], src1_inds2d[:, 0]]
                src2_feats = src2_feature2d[:, src2_inds2d[:, 1], src2_inds2d[:, 0]]
                tgt1_feats = tgt1_feature2d[:, tgt1_inds2d[:, 1], tgt1_inds2d[:, 0]]
                tgt2_feats = tgt2_feature2d[:, tgt2_inds2d[:, 1], tgt2_inds2d[:, 0]]
     ...
     # sift-feature
                ...
                # x=x.repeat(1,3)
                # # "=================================================================="
                # pcd=torch.cat((src_pcd_raw,tgt_pcd_raw),dim=0).detach().cpu()
                # src_pcd_raw=pcd[:src_pcd_raw.shape[0],:]
                # tgt_pcd_raw=pcd[src_pcd_raw.shape[0]:,:]
                #
                # tgt1_inds3d=tgt1_inds3d+src_pcd_raw.shape[0]
                # tgt2_inds3d=tgt2_inds3d+src_pcd_raw.shape[0]
                # pcd=torch.from_numpy(normalize_pcd(pcd))
                # src1_point_with_2dfeature=pcd[:src_pcd_raw.shape[0],:].cuda().detach()
                # tgt1_point_with_2dfeature=pcd[src_pcd_raw.shape[0]:,:].cuda().detach()
                # # src1_point_with_2dfeature=normalize_pcd(src1_point_with_2dfeature)
                # # src2_point_with_2dfeature=src_pcd_raw.cuda().detach()
                #
                # # tgt2_point_with_2dfeature=tgt_pcd_raw.cuda().detach()
                # #            print(f'x shape:{x.shape}')
                # #           print(f'src1 xyz shape:{src1_point_with_2dfeature.shape}')
                # #          print(f'tgt1 xyz shape:{tgt1_point_with_2dfeature.shape}')
                # x[:src_pcd_raw.shape[0],:]=src1_point_with_2dfeature
                # x[src_pcd_raw.shape[0]:,:]=tgt1_point_with_2dfeature
        ...
        if self.node_overlap:
            ...
            node_overlap_score_pred = torch.clamp(sigmoid(node_overlap_score_pred.view(-1)), min=0,
                                                  max=1)  # torch.clamp(sigmoid(node_overlap_score_pred.view(-1)),min=0,max=1)
