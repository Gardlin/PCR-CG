...
class IndoorTester(Trainer):
    ...
    def test(self):
        ...
        os.makedirs(f'{self.snapshot_dir}',exist_ok=True)
        os.makedirs(f'{self.snapshot_dir}/pth',exist_ok=True)

        num_iter = int(len(self.loader['test'].dataset) // self.loader['test'].batch_size)
        c_loader_iter = self.loader['test'].__iter__()
        self.model.eval()
        circle_recall=np.zeros(num_iter)
        with torch.no_grad():
            ...
            for idx in tqdm(range(num_iter)): # loop through this epoch
                ...
                if self.image_feature:
                    outputs = self.model(inputs,self.backbone2d)#[N1, C1], [N2, C2]
                else:
                    outputs=self.model(inputs)


                feats=outputs['feats_f']
                scores_overlap=outputs['scores_overlap']
                scores_saliency=outputs['scores_saliency']
                pcd = inputs['points'][0]
                ...
                ###################################################
                # get loss
                stats= self.desc_loss(loss_input)
                circle_recall[idx]=stats['recall'].detach().cpu().numpy()
                if idx%100==0:
                    print(f"mean recall: {np.sum(circle_recall)/(idx+1):.4f}")
                ...
                # print(data['pcd'][0,:],"data pcd")
                # print(data['feats'][0,:],'data feats')
                torch.save(data,f'{self.snapshot_dir}/pth/{idx}.pth')
            print("save directory: \n",f'{self.snapshot_dir}/pth')
